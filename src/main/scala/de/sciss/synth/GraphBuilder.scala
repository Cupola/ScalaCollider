...
// def outputs: Seq[ UGenIn ] = elements
   ...
   override def frac : GE	      = copy( outputs.flatMap( _.frac.outputs ))
   override def signum : GE	   = copy( outputs.flatMap( _.signum.outputs ))
   override def squared : GE     = copy( outputs.flatMap( _.squared.outputs ))
   override def cubed : GE       = copy( outputs.flatMap( _.cubed.outputs ))
   override def sqrt : GE        = copy( outputs.flatMap( _.sqrt.outputs ))
   override def exp : GE         = copy( outputs.flatMap( _.exp.outputs ))
   override def reciprocal : GE  = copy( outputs.flatMap( _.reciprocal.outputs ))
   override def midicps : GE     = copy( outputs.flatMap( _.midicps.outputs ))
   override def cpsmidi : GE     = copy( outputs.flatMap( _.cpsmidi.outputs ))
   override def midiratio : GE   = copy( outputs.flatMap( _.midiratio.outputs ))
   override def ratiomidi : GE   = copy( outputs.flatMap( _.ratiomidi.outputs ))
   override def dbamp : GE       = copy( outputs.flatMap( _.dbamp.outputs ))
   override def ampdb : GE       = copy( outputs.flatMap( _.ampdb.outputs ))
   override def octcps : GE      = copy( outputs.flatMap( _.octcps.outputs ))
   override def cpsoct : GE      = copy( outputs.flatMap( _.cpsoct.outputs ))
   override def log : GE         = copy( outputs.flatMap( _.log.outputs ))
   override def log2 : GE        = copy( outputs.flatMap( _.log2.outputs ))
   override def log10 : GE       = copy( outputs.flatMap( _.log10.outputs ))
   override def sin : GE         = copy( outputs.flatMap( _.sin.outputs ))
   override def cos : GE         = copy( outputs.flatMap( _.cos.outputs ))
   override def tan : GE         = copy( outputs.flatMap( _.tan.outputs ))
   override def asin : GE        = copy( outputs.flatMap( _.asin.outputs ))
   override def acos : GE        = copy( outputs.flatMap( _.acos.outputs ))
   override def atan : GE        = copy( outputs.flatMap( _.atan.outputs ))
   override def sinh : GE        = copy( outputs.flatMap( _.sinh.outputs ))
   override def cosh : GE        = copy( outputs.flatMap( _.cosh.outputs ))
   override def tanh : GE        = copy( outputs.flatMap( _.tanh.outputs ))
   override def distort : GE     = copy( outputs.flatMap( _.distort.outputs ))
   override def softclip : GE    = copy( outputs.flatMap( _.softclip.outputs ))
}
