...
case class RandID( rate: Rate, id: UGenIn ) extends SingleOutUGen( id )

object Rand extends UGen2RArgs {
	def ir( lo: GE = 0, hi: GE = 1 ) : GE = make( lo, hi )
}
case class Rand( lo: UGenIn, hi: UGenIn ) extends SingleOutUGen( lo, hi )
with ScalarRated

object IRand extends UGen2RArgs {
	def ir( lo: GE = 0, hi: GE = 127 ) : GE = make( lo, hi )
}
case class IRand( lo: UGenIn, hi: UGenIn ) extends SingleOutUGen( lo, hi )
with ScalarRated

object TRand extends UGen3Args {
	def ar( lo: GE = 0, hi: GE = 1, trig: GE ) : GE = arExp( lo, hi, trig )
	def kr( lo: GE = 0, hi: GE = 1, trig: GE ) : GE = krExp( lo, hi, trig )
}
case class TRand( rate: Rate, lo: UGenIn, hi: UGenIn, trig: UGenIn )
extends SingleOutUGen( lo, hi, trig )

object TIRand extends UGen3Args {
	def ar( lo: GE = 0, hi: GE = 127, trig: GE ) : GE = arExp( lo, hi, trig )
	def kr( lo: GE = 0, hi: GE = 127, trig: GE ) : GE = krExp( lo, hi, trig )
}
case class TIRand( rate: Rate, lo: UGenIn, hi: UGenIn, trig: UGenIn )
extends SingleOutUGen( lo, hi, trig )

object LinRand extends UGen3RArgs {
	def ir( lo: GE = 0, hi: GE = 127, minMax: GE = 0 ) : GE =
      make( lo, hi, minMax )
}
case class LinRand( lo: UGenIn, hi: UGenIn, minMax: UGenIn )
extends SingleOutUGen( lo, hi, minMax ) with ScalarRated

object NRand extends UGen3RArgs {
	def ir( lo: GE = 0, hi: GE = 127, n: GE = 0 ) : GE =
      make( lo, hi, n )
}
case class NRand( lo: UGenIn, hi: UGenIn, n: UGenIn )
extends SingleOutUGen( lo, hi, n ) with ScalarRated

object ExpRand extends UGen2RArgs {
	def apply( lo: GE = 0.01f, hi: GE = 1 ) : GE = make( lo, hi )
}
case class ExpRand( lo: UGenIn, hi: UGenIn )
extends SingleOutUGen( lo, hi ) with ScalarRated

object TExpRand extends UGen3Args {
	def ar( lo: GE = 0.01f, hi: GE = 1, trig: GE ) : GE = arExp( lo, hi, trig )
	def kr( lo: GE = 0.01f, hi: GE = 1, trig: GE ) : GE = krExp( lo, hi, trig )
}
case class TExpRand( rate: Rate, lo: UGenIn, hi: UGenIn, trig: UGenIn )
extends SingleOutUGen( lo, hi, trig )

object CoinGate extends UGen2Args {
...
object LFNoise0 extends UGen1Args {
	def ar( freq: GE = 500 ) : GE = arExp( freq )
	def kr( freq: GE = 500 ) : GE = krExp( freq )
}
case class LFNoise0( rate: Rate, freq: UGenIn ) extends SingleOutUGen( freq )

object LFNoise1 extends UGen1Args {
	def ar( freq: GE = 500 ) : GE = arExp( freq )
	def kr( freq: GE = 500 ) : GE = krExp( freq )
}
case class LFNoise1( rate: Rate, freq: UGenIn ) extends SingleOutUGen( freq )

object LFNoise2 extends UGen1Args {
	def ar( freq: GE = 500 ) : GE = arExp( freq )
	def kr( freq: GE = 500 ) : GE = krExp( freq )
}
case class LFNoise2( rate: Rate, freq: UGenIn ) extends SingleOutUGen( freq )

object LFClipNoise extends UGen1Args {
	def ar( freq: GE = 500 ) : GE = arExp( freq )
	def kr( freq: GE = 500 ) : GE = krExp( freq )
}
case class LFClipNoise( rate: Rate, freq: UGenIn ) extends SingleOutUGen( freq )

object LFDNoise0 extends UGen1Args {
	def ar( freq: GE = 500 ) : GE = arExp( freq )
	def kr( freq: GE = 500 ) : GE = krExp( freq )
}
case class LFDNoise0( rate: Rate, freq: UGenIn ) extends SingleOutUGen( freq )

object LFDNoise1 extends UGen1Args {
	def ar( freq: GE = 500 ) : GE = arExp( freq )
	def kr( freq: GE = 500 ) : GE = krExp( freq )
}
case class LFDNoise1( rate: Rate, freq: UGenIn ) extends SingleOutUGen( freq )

object LFDNoise3 extends UGen1Args {
	def ar( freq: GE = 500 ) : GE = arExp( freq )
	def kr( freq: GE = 500 ) : GE = krExp( freq )
}
case class LFDNoise3( rate: Rate, freq: UGenIn ) extends SingleOutUGen( freq )

object LFDClipNoise extends UGen1Args {
	def ar( freq: GE = 500 ) : GE = arExp( freq )
	def kr( freq: GE = 500 ) : GE = krExp( freq )
}
case class LFDClipNoise( rate: Rate, freq: UGenIn ) extends SingleOutUGen( freq )
...
object Dust extends UGen1Args {
	def ar( density: GE = 1 ) : GE = arExp( density )
	def kr( density: GE = 1 ) : GE = krExp( density )
}
case class Dust( rate: Rate, density: UGenIn ) extends SingleOutUGen( density )

object Dust2 extends UGen1Args {
	def ar( density: GE = 1 ) : GE = arExp( density )
	def kr( density: GE = 1 ) : GE = krExp( density )
}
case class Dust2( rate: Rate, density: UGenIn ) extends SingleOutUGen( density )
