...
object Predef {
  ...
  implicit def seqOfGE2GESeq( x: Seq[ GE ]) = new GESeq( x flatMap (_.toUGenInputs) )

	// why these are necessary now??
  implicit def seqOfFloat2GESeq( x: Seq[ Float ]) = new GESeq( x map (Constant( _ ) ))
  implicit def seqOfInt2GESeq( x: Seq[ Int ]) = new GESeq( x map (i => Constant( i.toFloat )))
  implicit def seqOfDouble2GESeq( x: Seq[ Double ]) = new GESeq( x map (d => Constant( d.toFloat )))
...
//  var res = new Array[T]( num );
  ...
  def ampdb( amp: Float ) = log( amp ) / log( 10 ) * 20
...
//	  play( f, target, 0, Some(0.02f), 'addToHead )
  ...
  def play( f: => GE, target: => Node, outBus: Int, fadeTime: Option[Float], addAction: Symbol ) : Synth = {
	def func() = f
...
//		}
		val synthDef = GraphBuilder.wrapOut( "temp__" + abs( func.hashCode ).toString, func, Nil, Nil, "Out", fadeTime );
