...
ScalaCollider currently compiles against Scala 2.8.0.RC2. Note that RC3 currently has a bug that prevents ScalaCollider from compiling. It requires Java 1.6 and SuperCollider 3.3.1+. It depends on ScalaOSC ( "github.com/Sciss/ScalaOSC":http://github.com/Sciss/ScalaOSC ) and ScalaAudioFile ( "github.com/Sciss/ScalaAudioFile":http://github.com/Sciss/ScalaAudioFile
...
The demo target starts a REPL and executes the file @console-startup.txt@ which by default imports the necessary packages, creates a server-options builder bound tot @so@ and defines a @boot@ method. make sure that @so.programPath@ is set correctly:
